        ...
        for i in range(0, self.doc.page_count-1):           
            ...
            try:
                ...
                for j in range(len(titles)):
                    courses.append(Course(titles[j], descriptions[j], target_groups[j], contents[j], prerequisites[j], dates_locations[j], times[j], costs[j], trainers[j], additional_infos[j], what_to_bring[j], categories[j], min_ages[j], durations[j], durations_days[j]))
